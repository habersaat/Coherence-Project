...
module top_mesi_wl1 (
    ...
    initial begin
        // Workload 1: Producer-Consumer (shared counter)
        memory[0] = 32'hff010113;  // addi sp, sp, -16
        memory[1] = 32'h00812623;  // sw   ra, 12(sp)
        memory[2] = 32'h01010413;  // addi s0, sp, 16
        memory[3] = 32'h10002783;  // lw   a5, 0x100(zero)
        memory[4] = 32'h00178713;  // addi a4, a5, 1
        memory[5] = 32'h10e02023;  // sw   a4, 0x100(zero)
        memory[6] = 32'hff5ff06f;  // jal  zero, -12
    end
...
module top_mesi_wl6 (
    ...
    // -------------------------------------------------------------------------
    // Bus request outputs from each cache controller
old timer counts how long BTNC has been continuously held.
