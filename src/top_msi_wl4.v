...
module top_mesi_wl4 (
    ...
    initial begin
        ...
        // Each core sums its 4-element chunk (0x300-0x33C), accumulates
        // into shared result at 0x100. Private reads (S-state hits) +
        // shared write (coherence contention on accumulator).
        ...
        // Main loop (0x024):
        memory[ 9] = 32'h0004a903;  // lw   s2, 0(s1)          [main loop]
        memory[10] = 32'h0044a283;  // lw   t0, 4(s1)
        memory[11] = 32'h00590933;  // add  s2, s2, t0
        memory[12] = 32'h0084a283;  // lw   t0, 8(s1)
        memory[13] = 32'h00590933;  // add  s2, s2, t0
        memory[14] = 32'h00c4a283;  // lw   t0, 12(s1)
        memory[15] = 32'h00590933;  // add  s2, s2, t0
        memory[16] = 32'h10002283;  // lw   t0, 0x100(zero)
        memory[17] = 32'h012282b3;  // add  t0, t0, s2
        memory[18] = 32'h10502023;  // sw   t0, 0x100(zero)
        memory[19] = 32'hfd9ff06f;  // jal  zero, -40
    ...
    always @(posedge CLK100MHZ) begin
        ...
        if (bus_trans_pending && !bus_snoop_busy) begin
            bus_resp_valid <= 1;
            // Address 0x3FC: return hardware core ID to requesting core.
            // Race-free core identification - each core reads a unique value
            // because the latched grant selects which ID to return.
            if (bus_trans_addr == 32'h3FC)
                bus_resp_data <= grant[0] ? 32'd0 :
                                 grant[1] ? 32'd1 :
                                 grant[2] ? 32'd2 :
                                 grant[3] ? 32'd3 : 32'd0;
            else
    ...
    // silent_upgrade: pulses when a write hits E-state line (E->M, no bus tx).
    // -------------------------------------------------------------------------
    ...
    wire c0_silent_upgrade, c1_silent_upgrade, c2_silent_upgrade, c3_silent_upgrade;
    ...
    cache_mesi dut0 (
        ...
        .bus_resp_excl(bus_resp_excl), .silent_upgrade(c0_silent_upgrade)
    );
    cache_mesi dut1 (
        ...
        .bus_resp_excl(bus_resp_excl), .silent_upgrade(c1_silent_upgrade)
    );
    cache_mesi dut2 (
        ...
        .bus_resp_excl(bus_resp_excl), .silent_upgrade(c2_silent_upgrade)
    );
    cache_mesi dut3 (
        ...
        .bus_resp_excl(bus_resp_excl), .silent_upgrade(c3_silent_upgrade)
    );
    ...
    // OR of all four silent_upgrade pulses - any core silently upgrading E->M
    wire any_silent_upgrade = c0_silent_upgrade | c1_silent_upgrade
                            | c2_silent_upgrade | c3_silent_upgrade;
    reg  su_prev;
    always @(posedge CLK100MHZ) su_prev <= any_silent_upgrade;
    wire su_rising = any_silent_upgrade && !su_prev;

    reg [31:0] cnt_busrd, cnt_busrdx, cnt_busupgr, cnt_interv, cnt_inval, cnt_e_upgrade;
    always @(posedge CLK100MHZ) begin
        if (!CPU_RESETN) begin
            cnt_busrd   <= 0;
            cnt_busrdx  <= 0;
            ...
            if (su_rising) cnt_e_upgrade <= cnt_e_upgrade + 1;
        end
    end
...
module top_msi_wl3 (
    ...
    // -------------------------------------------------------------------------
    // Bus transaction type encoding (matches cache_msi.v localparams)
    // -------------------------------------------------------------------------
    localparam BUS_RD   = 2'd0;
    localparam BUS_RDX  = 2'd1;
    localparam BUS_UPGR = 2'd2;
    ...
    // -------------------------------------------------------------------------
    // PicoRV32 memory interface signals - one set per core.
    // PicoRV32's mem_* ports connect directly to cache_msi's core_* ports.
    // mem_instr is unused by the cache (no instruction/data distinction).
    // -------------------------------------------------------------------------
    wire        c0_cpu_valid, c1_cpu_valid, c2_cpu_valid, c3_cpu_valid;
    wire        c0_cpu_instr, c1_cpu_instr, c2_cpu_instr, c3_cpu_instr;
    ...
    // -------------------------------------------------------------------------
    // Direct memory-facing signals (per core, writeback path only).
    // cache_msi only issues wstrb=1111 writes on this interface (WRITEBACK
    // and MI states). All read data comes back through bus_resp_data instead.
    // mem_rdata is tied to 0 - caches never read on this path in MSI.
    // -------------------------------------------------------------------------
    wire        c0_mem_valid, c1_mem_valid, c2_mem_valid, c3_mem_valid;
...
@(posedge CLK100MHZ) begin
    ...
    reg [31:0] cap_busrd, cap_busrdx, cap_busupgr, cap_interv, cap_inval, cap_e_upgrade;

    // Frozen results - latched simultaneously at end of capture window
    reg [31:0] frz_busrd, frz_busrdx, frz_busupgr, frz_interv, frz_inval, frz_e_upgrade;
            ...
            case (cap_state)
                CAP_LIVE: begin
                    if (long_press) begin
                        ...
                        cap_timer   <= 0;
                        cap_busrd   <= 0; cap_busrdx  <= 0; cap_busupgr <= 0;
                        cap_interv  <= 0; cap_inval   <= 0; cap_e_upgrade <= 0;
                ...
                CAP_CAPTURING: begin
                    ...
                    if (su_rising) cap_e_upgrade <= cap_e_upgrade + 1;
        ...
        case (cap_state)
            CAP_LIVE: begin
                case (cnt_sel)
                    ...
                    3'd5: cnt_display = cnt_e_upgrade;
            ...
            CAP_CAPTURING: begin
                case (cnt_sel)
                    ...
                    3'd5: cnt_display = cap_e_upgrade;
                    default: cnt_display = 32'd0;
                endcase
            end
            CAP_FROZEN: begin
                case (cnt_sel)
                    ...
                    3'd5: cnt_display = frz_e_upgrade;
                    default: cnt_display = 32'd0;
                endcase
            end
            default: cnt_display = 32'd0;
    ...
    assign LED[5] = (cap_state == CAP_LIVE)   ? (cnt_sel == 3'd5) :
                    (cap_state == CAP_CAPTURING) ? blink :
                    /* FROZEN */                  (cnt_sel == 3'd5) && blink;
