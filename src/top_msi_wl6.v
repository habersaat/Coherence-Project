...
module top_msi_wl6 (
    ...
    // OR of all snoop_busy outputs. snoop_busy is combinational in cache_msi
    ...
    initial begin
        // Workload 6: Single-core read-modify-write loop (eliminates all token complexity)
        //
        // Only core 0 accesses 0x100. Cores 1,2,3 idle.
        // Key: sw instruction at 0x048 is idx 2, data at 0x100 is idx 0.
        // No instruction between lw and sw touches idx 0 -> counter stays in S/E.
        //   MSI: lw gets S -> sw issues BusUpgr (1 per loop iteration)
        //   MESI: lw gets E (sole reader) -> sw is silent E->M (0 BusUpgr)
        //
        // Landing pads / idle loops:
        memory[ 0] = 32'h00000413;  // 0x000: addi s0,zero,0  (core 0 ID)
        memory[ 1] = 32'h03c0006f;  // 0x004: jal  zero,+60   -> 0x040
        ...
        // Core 0 main loop at 0x040 (word 16):
        memory[16] = 32'h10002283;  // 0x040: lw   t0,0x100(zero)  idx 0 data
        memory[17] = 32'h00128293;      // 0x044: addi t0,t0,1
        memory[18] = 32'h10502023;  // 0x048: sw   t0,0x100(zero)  idx 2 instr, idx 0 data
        memory[19] = 32'hff5ff06f;               // 0x04C: jal  zero,-12        -> 0x040
        memory[64] = 32'd0;  // counter at 0x100
    end
    ...
    //
    // The one-cycle gap is essential: snoop_busy is combinational in cache_msi
    ...
    // -------------------------------------------------------------------------
    // cache_msi instantiations - one per core
    // -------------------------------------------------------------------------
    cache_msi dut0 (
    ...
    //               starts (combinational snoop_busy from cache_msi).
